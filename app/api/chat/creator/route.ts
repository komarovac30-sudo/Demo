...
export async function GET(req: NextRequest) {
  ...
  const latest = new Map<string, { message: string; created_at: string; sender_type: string }>();
  if (ids.length) {
    const { data: messages } = await admin.from("chat_messages")
      .select("thread_id,message,created_at,sender_type")
      .in("thread_id", ids)
      .gt("expires_at", new Date().toISOString())
      .order("created_at", { ascending: false })
      .limit(400);
    for (const item of messages || []) if (!latest.has(item.thread_id)) latest.set(item.thread_id, item);
  }
